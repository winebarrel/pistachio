...
func TestObjectCount_SchemaLabel(t *testing.T) {
	c := ObjectCount{Schemas: []string{"public"}}
	assert.Equal(t, "schema public", c.SchemaLabel())

	c2 := ObjectCount{Schemas: []string{"public", "myschema"}}
	assert.Equal(t, "schemas public, myschema", c2.SchemaLabel())
}

func TestObjectCount_Summary(t *testing.T) {
	c := ObjectCount{Tables: 3, Views: 1, Enums: 2, Domains: 0, CompositeTypes: 0, Sequences: 4}
...
func TestObjectCount_Summary_Singular(t *testing.T) {
	c := ObjectCount{Tables: 1, Views: 1, Enums: 1, Domains: 1, CompositeTypes: 1, Sequences: 1}
