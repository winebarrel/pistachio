...
lobechat|sample-db-lobechat||lobechat
endef
...
# LobeChat (lobehub/lobe-chat, Apache-2.0), the chat front end for the LLM
# providers. The schema ships as Drizzle migrations like dokploy's and loads
# the same way: the repository tarball is fetched once, only the migrations
# directory is extracted, and the tags are read out of meta/_journal.json
# rather than off the directory, since here too the two do not agree.
# 0065_add_document_fields.sql is on disk but not in the journal, left behind
# by the rename that made it 0066_add_document_fields.sql, and the two differ
# in the ON DELETE action of the foreign key they add, so replaying the
# directory would take the older one. A few files end without a semicolon, so
# each is followed by a newline and one, and the `public` qualifier Drizzle
# writes into a foreign key's target is stripped the way sample-db-prisma
# strips it, along with the one the `to_regclass` and `::regclass` guards in a
# handful of hand-written migrations carry.
#
# The vectors are why it is here, and the first of the two reasons it does not
# share dokploy's loader. LobeChat keeps what it remembers about a user as
...
LOBECHAT_SHA = 14dfc07b14eee1984e52c195df9319636d6b167b

sample-db-lobechat: PGOPTS = -c search_path=lobechat,public
.PHONY: sample-db-lobechat
sample-db-lobechat:
	$(PSQL) -c 'CREATE EXTENSION IF NOT EXISTS vector WITH SCHEMA public'
	$(PSQL) -c 'CREATE SCHEMA IF NOT EXISTS lobechat'
	dir=$$(mktemp -d) && trap 'rm -rf "$$dir"' EXIT && \
	curl -sSfL --retry 3 --retry-delay 2 https://codeload.github.com/lobehub/lobe-chat/tar.gz/$(LOBECHAT_SHA) \
	  | tar xz -C "$$dir" --strip-components=4 lobe-chat-$(LOBECHAT_SHA)/packages/database/migrations && \
	cd "$$dir" && \
	grep -oE '"tag": "[^"]+"' meta/_journal.json | sed 's/.*: "//; s/"$$//' \
	  | while read -r t; do \
