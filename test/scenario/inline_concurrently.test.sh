#!/usr/bin/env bash
# Scenario test: inline CREATE INDEX CONCURRENTLY (without -- pista:concurrently directive)
set -euo pipefail
...
if ! echo "$plan_output" | grep -qF 'DROP INDEX CONCURRENTLY'; then
  ...
  apply_output=$(pista_apply "$DATA/steps/02_change_index.sql") || { fail "apply failed: $apply_output"; true; }
  drift=$(pista_plan "$DATA/steps/02_change_index.sql") || { fail "drift check failed: $drift"; true; }
  if echo "$drift" | grep -q 'No changes'; then
    pass
  else
    fail "drift after apply"
...
plan_output=$(pista_plan "$DATA/steps/03_add_unique.sql") || { fail "plan failed: $plan_output"; true; }
if ! echo "$plan_output" | grep -qF 'CREATE UNIQUE INDEX CONCURRENTLY'; then
  fail "expected CREATE UNIQUE INDEX CONCURRENTLY in plan"
  echo "    $plan_output" >&2
else
  apply_output=$(pista_apply "$DATA/steps/03_add_unique.sql") || { fail "apply failed: $apply_output"; true; }
  drift=$(pista_plan "$DATA/steps/03_add_unique.sql") || { fail "drift check failed: $drift"; true; }
  if echo "$drift" | grep -q 'No changes'; then
    pass
  else
    fail "drift after apply"
...
plan_output=$(pista_plan "$DATA/steps/04_matview_index.sql") || { fail "plan failed: $plan_output"; true; }
if ! echo "$plan_output" | grep -qF 'CREATE INDEX CONCURRENTLY idx_user_names'; then
  ...
  apply_output=$(pista_apply "$DATA/steps/04_matview_index.sql") || { fail "apply failed: $apply_output"; true; }
  drift=$(pista_plan "$DATA/steps/04_matview_index.sql") || { fail "drift check failed: $drift"; true; }
  if echo "$drift" | grep -q 'No changes'; then
    pass
  else
    fail "drift after apply"
...
apply_output=$("$PISTA" apply --allow-drop all --with-tx "$DATA/steps/01_add_index.sql" 2>&1) && {
