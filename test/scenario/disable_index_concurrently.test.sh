#!/usr/bin/env bash
# Scenario test: --disable-index-concurrently flag overrides -- pista:concurrently directive
set -euo pipefail
...
pista_plan_disable() {
  "$PISTA" plan --allow-drop all --disable-index-concurrently "$@" 2>&1
}

pista_apply_disable() {
  "$PISTA" apply --allow-drop all --disable-index-concurrently "$@" 2>&1
}
...
plan_output=$(pista_plan_disable "$DATA/steps/01_add_index.sql") || { fail "plan failed: $plan_output"; true; }
if ! echo "$plan_output" | grep -qF 'CREATE INDEX idx_users_name'; then
  ...
  apply_output=$(pista_apply_disable "$DATA/steps/01_add_index.sql") || { fail "apply failed: $apply_output"; true; }
  drift=$(pista_plan_disable "$DATA/steps/01_add_index.sql") || { fail "drift check failed: $drift"; true; }
...
plan_output=$(pista_plan_disable "$DATA/steps/02_change_index.sql") || { fail "plan failed: $plan_output"; true; }
if ! echo "$plan_output" | grep -qE 'DROP INDEX (public\.)?idx_users_name;'; then
  ...
  apply_output=$(pista_apply_disable "$DATA/steps/02_change_index.sql") || { fail "apply failed: $apply_output"; true; }
  drift=$(pista_plan_disable "$DATA/steps/02_change_index.sql") || { fail "drift check failed: $drift"; true; }
  if echo "$drift" | grep -q 'No changes'; then
    pass
  else
    fail "drift after apply"
...
step "03 drop index"
plan_output=$(pista_plan_disable "$DATA/steps/03_drop_index.sql") || { fail "plan failed: $plan_output"; true; }
if ! echo "$plan_output" | grep -qE 'DROP INDEX (public\.)?idx_users_name;'; then
  fail "expected DROP INDEX idx_users_name in plan"
  echo "    $plan_output" >&2
  ...
  apply_output=$(pista_apply_disable "$DATA/steps/03_drop_index.sql") || { fail "apply failed: $apply_output"; true; }
  drift=$(pista_plan_disable "$DATA/steps/03_drop_index.sql") || { fail "drift check failed: $drift"; true; }
  if echo "$drift" | grep -q 'No changes'; then
    pass
  else
    fail "drift after apply"
...
plan_output=$(pista_plan_disable "$DATA/steps/04_matview_index.sql") || { fail "plan failed: $plan_output"; true; }
if ! echo "$plan_output" | grep -qF 'CREATE INDEX idx_user_names'; then
  ...
  apply_output=$(pista_apply_disable "$DATA/steps/04_matview_index.sql") || { fail "apply failed: $apply_output"; true; }
  drift=$(pista_plan_disable "$DATA/steps/04_matview_index.sql") || { fail "drift check failed: $drift"; true; }
  if echo "$drift" | grep -q 'No changes'; then
    pass
  else
    fail "drift after apply"
...
apply_output=$("$PISTA" apply --allow-drop all --disable-index-concurrently --with-tx "$DATA/steps/05_with_tx.sql" 2>&1) || { fail "apply failed: $apply_output"; true; }
if ! echo "$apply_output" | grep -qF 'CREATE UNIQUE INDEX idx_users_email'; then
  ...
  drift=$(pista_plan_disable "$DATA/steps/05_with_tx.sql") || { fail "drift check failed: $drift"; true; }
  if echo "$drift" | grep -q 'No changes'; then
    pass
  else
    fail "drift after apply"
...
apply_output=$("$PISTA" apply --allow-drop all --with-tx "$DATA/steps/05_with_tx.sql" 2>&1) && {
