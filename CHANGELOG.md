...
* `plan --out` writes the plan to a file, and `apply-from` executes it. The statements are fixed when the file is written: `apply-from` reads no schema file and diffs nothing, it reads the database only to check that it is still the state the plan was computed against, and stops when it is not. `--force` applies over that with a warning. The file records the options that decide what the plan read, so `apply-from` reads the database the way the plan did, and a `-- pista:execute` check is decided once, when the file is written.
...
* Manage a comment on a view or materialized view column. `dump` did not write one, and a `COMMENT ON COLUMN` naming one was dropped without a warning.
* An index on a partitioned table written without `ONLY` is no longer re-created on every plan, and an index attached to it is not dropped on its own. That `DROP INDEX` always failed.
