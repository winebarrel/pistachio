...
* `fmt` and `dump` keep an `ARRAY[...]` constructor on one line. The layout counted a parenthesis but not a square bracket, so a comma inside a constructor read as the end of a column and broke the line after it: `DEFAULT ARRAY['x'::text,` followed by `'y'::text],`.
