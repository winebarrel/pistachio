...
* An index on a partitioned table written without `ONLY` no longer re-creates on every plan, and an index attached to it is not dropped on its own. That `DROP INDEX` always failed, even when the parent's index was dropped in the same plan.
