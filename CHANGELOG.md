...
* `--schema-map` now reaches a column's type, a column's default and a partition's parent. `dump -m myschema=public` wrote `s myschema.status`, `DEFAULT nextval('myschema.seq1'::regclass)` and `PARTITION OF myschema.parent` under a `public` table, which does not load, and `plan` retyped such a column and re-set such a default on every run. The map also no longer rewrites a schema whose name merely ends in the mapped one: a foreign key to `mystaging.ref` came out as `mypublic.ref` under `-m staging=public`.
