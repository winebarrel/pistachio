...
- openreplay and logto 2026-09-20 on 16.13, and omero 2026-09-21 on 15.18.
...
| **Total** | **8,901** | **76,385** | **27,181** | **11,582** | **14,768** | **2,253** | **634** | **639** | **1,729** | **1,351** |

### Size

The 90 dumps come to about 250,000 lines of SQL. chado is 43,700 of them, the
...
- **Triggers**: boundary's 741, more than any other sample, spread over 182
  ...
  constraint triggers that guard a privilege change.
...
Routines are concentrated the same way. Forty-two of the 90 samples declare
one at all, and gitlab's 337, boundary's 225, kea's and musicbrainz's 130 each,
and chado's 94 are 916 of the 1,351. Seven in ten of them, 952, return
...
1,244 are written in plpgsql and 107 in sql. omero's 57, the next largest
after lemmy's 74, are 56 of the plpgsql and one of the sql, and 49 of them
return `trigger`. sourcegraph declares one
...
definitions, chado's nine unloadable functions and the `search_path` two of
logto's functions are declared with aside, so the round trip still covers the
full schema.
