...
Pass `--search-path` (env `$PISTA_SEARCH_PATH`) to change it. `--search-path=` qualifies everything, so the dump reloads the same way wherever it runs. `--search-path=myschema` drops `myschema` from the output, and makes a desired schema that qualifies its own objects plan on every run. The default keeps `"$user"`, so a schema named after the connecting role is reported without its schema; `--search-path=public` covers that case.
