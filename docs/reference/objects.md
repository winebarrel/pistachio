...
A definition change is applied with `CREATE OR REPLACE TRIGGER`, which takes a lighter lock than dropping and recreating. PostgreSQL has no `CREATE OR REPLACE CONSTRAINT TRIGGER`, so a change that involves a constraint trigger on either side runs as `DROP TRIGGER` and `CREATE TRIGGER` and needs `--allow-drop trigger`, the same as any other trigger drop. Without it the trigger keeps its current definition.
