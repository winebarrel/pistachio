...
A change to a primary key, unique or exclusion constraint, or to an index, is a drop and an add. PostgreSQL refuses the drop while a foreign key references the key or the index, or a view relies on the primary key in its `GROUP BY`. `plan` fails first and names them:

```
pista: error: cannot drop constraint t_pkey on public.t: foreign key r_a_fkey on public.r depends on it
```

A foreign key or a view the same plan drops does not block, since those drops run first. To change a key a foreign key references, change the foreign key in the same run, or drop it in one run and add it back in the next.
