...
func diffTable(current, desired *model.Table, dc DropChecker) (*tableDiffResult, error) {
	...
	// RENAME COLUMN transparently to indexes/constraints/FKs/triggers on the
	// same table. Also rekey current.Columns so diffComments looks up the
	// right entry by the new column name.
