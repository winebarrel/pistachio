...
func addColumnSQL(fqtn string, col *model.Column) string {
	...
	if col.Identity.IsGeneratedAlways() {
		sql += " GENERATED ALWAYS AS IDENTITY" + col.IdentitySeq.OptionsSQL(col.TypeName)
	} else if col.Identity.IsGeneratedByDefault() {
		sql += " GENERATED BY DEFAULT AS IDENTITY" + col.IdentitySeq.OptionsSQL(col.TypeName)
...
func alterColumnSQL(fqtn string, current, desired *model.Column) []string {
	...
	case curIsIdent && desIsIdent:
		...
		// One statement takes both, and PostgreSQL checks the result as a
		// whole, so the clauses need no particular order.
		var clauses []string
		if current.Identity != desired.Identity {
			clauses = append(clauses, "SET GENERATED "+identityKind(desired.Identity))
		}
		// A SET DATA TYPE above already moved the bounds that sat at the old
		// type's default, so the current side is read as it is after it.
		currentSeq := current.IdentitySeq
		if retyped {
			currentSeq = currentSeq.RetypedTo(current.TypeName, desired.TypeName)
		}
		clauses = append(clauses, identitySeqClauses(currentSeq, desired.IdentitySeq)...)
		if len(clauses) > 0 {
			stmts = append(stmts, "ALTER TABLE "+fqtn+" ALTER COLUMN "+colIdent+" "+strings.Join(clauses, " ")+";")
		}
	}
...
// identitySeqClauses returns a SET clause per changed sequence option of an
// identity column. Both sides carry every value resolved, so an option the
// desired definition leaves out reads as its default and is reset here.
...
func identitySeqClauses(current, desired *model.IdentitySequence) []string {
	...
	if current.Start != desired.Start {
		clauses = append(clauses, "SET START WITH "+strconv.FormatInt(desired.Start, 10))
	}
	if current.Increment != desired.Increment {
		clauses = append(clauses, "SET INCREMENT BY "+strconv.FormatInt(desired.Increment, 10))
	}
	if current.Min != desired.Min {
		clauses = append(clauses, "SET MINVALUE "+strconv.FormatInt(desired.Min, 10))
	}
	if current.Max != desired.Max {
		clauses = append(clauses, "SET MAXVALUE "+strconv.FormatInt(desired.Max, 10))
	}
	if current.Cache != desired.Cache {
		clauses = append(clauses, "SET CACHE "+strconv.FormatInt(desired.Cache, 10))
	}
	if current.Cycle != desired.Cycle {
		if desired.Cycle {
			clauses = append(clauses, "SET CYCLE")
		} else {
			clauses = append(clauses, "SET NO CYCLE")
		}
	}
